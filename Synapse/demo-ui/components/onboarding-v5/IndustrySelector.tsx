...
export const IndustrySelector: React.FC<IndustrySelectorProps> = ({
  ...
  const handleCodeDetectionConfirm = async () => {
    ...
    // Check if we already have a profile for this code
    const existingIndustry = IndustryMatchingService.getByCode(detectedCode.naics_code);

    if (existingIndustry && existingIndustry.has_full_profile) {
      // Profile exists - use it immediately
  ...
  const handleSelect = async (industry: IndustryOption) => {
    ...
    // Check if we need to generate profile on-demand
    if (!industry.hasFullProfile) {
      ...
      // Start generation immediately (no confirmation dialog)
      await performGeneration(industry);
      return;
    } else {
      // Profile exists, just set it in state
      // When user clicks submit, CleanUrlEntry will detect hasFullProfile and skip scanning
                  ...
                  {industries.map(industry => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="industry-name">{industry.displayName}</span>
                        {industry.hasFullProfile && (
                          <span className="profile-badge" title="Pre-generated profile available">
                            ✓
                          </span>
                        )}
                        {!industry.hasFullProfile && (
                          <span className="ondemand-badge" title="Profile will be generated on-demand">
                            ⚡
                          </span>
                        )}
