...
export function PublishingQueue({
  ...
  const handleReschedule = async (item: ContentItemType) => {
    const newTime = prompt('Enter new date/time (YYYY-MM-DD HH:MM):', item.scheduled_time);
    if (!newTime) return;
    ...
    <Card className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Upcoming {days} days • {queueItems.length} scheduled posts
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">No scheduled posts in the next {days} days</p>
        </div>
      ) : (
                    ...
                    <Card key={queueItem.content.id} className="p-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Content Preview */}
                          <ContentItem item={queueItem.content} compact />
