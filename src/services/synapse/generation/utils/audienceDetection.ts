...
export function detectTargetAudience(
  ...
  if ('targetAudience' in business && business.targetAudience) {
    const audience = business.targetAudience;

    // Clean up comma-separated lists or invalid formats
    if (audience.includes(',') && audience.split(',').length > 3) {
      // This looks like a comma-separated list of event types, not a target audience
      // Try to infer actual audience from industry instead
      console.warn('[audienceDetection] targetAudience contains comma-separated list, inferring from industry:', audience);
    } else {
      return audience;
    }
  }
